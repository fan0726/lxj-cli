...
> ⚠️ **运行需要已安装 Node.js (>=18)。** 因为是纯 JS 脚本，**同一个 `lxj-c` 文件在 macOS / Linux / WSL 上都能用**，无需区分平台或 CPU 架构。

## 指令一览

| 指令 | 说明 |
| --- | --- |
| `lxj-c -v` / `lxj-c --version` | 显示版本号 |
| `lxj-c -h` / `lxj-c --help` | 显示所有可用指令 |
| `lxj-c wtr` | 获取天气预报（默认上海） |
| `lxj-c wtr <城市>` | 按城市名获取天气，如 `lxj-c wtr 北京` |
| `lxj-c wtr <城市> --json` | 返回原始 JSON |
...
# 直接用 node 跑（开发时）
node bin/lxj-c.js -v
node bin/lxj-c.js wtr
node bin/lxj-c.js wtr 广州
```
...
```
dist/
└── lxj-c     # 单文件 Node 脚本（几十 KB），需目标机器已装 Node
```
...
```bash
npm run build          # 生成 dist/lxj-c
gh release upload v1.0.0 dist/lxj-c install.sh --clobber   # 覆盖到已有 Release
# 或新建一个版本：
# gh release create v1.1.0 dist/lxj-c install.sh --title "lxj-c v1.1.0" --notes "..."
```
...
```bash
curl -fsSL https://github.com/fan0726/lxj-cli/releases/latest/download/lxj-c -o /usr/local/bin/lxj-c
chmod +x /usr/local/bin/lxj-c
...
```bash
gh release download -R fan0726/lxj-cli -p lxj-c
```
...
2. 在 `src/index.js` 的 `commands` 注册表里加一条：
   ...
   const commands = {
     wtr: { /* ... */ },
     news: { handler: news, usage: 'lxj-c news', summary: '获取新闻' },
   };
   ```
